...
const reducer = createReducer(initialAppState, {
	 SELECT_RESTAURANT(state, action: PayloadAction<Restaurant>){ state.selectedRestaurant = action.payload},
	 SELECT_APPLICATION(state, action: PayloadAction<Application>){ state.selectedApplication = action.payload},
	 LOAD_RESTAURANTS(state, action: PayloadAction<Application[]>){ state.restaurants = reduceApplicationsIntoRestaurants(action.payload)},
})
